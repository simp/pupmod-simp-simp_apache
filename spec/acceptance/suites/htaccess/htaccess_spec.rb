  ...
  describe "htaccess type/provider for #{host}" do
    ...
    it 'requires file resource' do
      apply_manifest_on(host, manifest1, expect_failures: true) do
        expect(stderr).to match(%r{You must declare a 'file' object to manage /root/htaccess.txt!}m)
      end
    end
    ...
    it 'works with no errors' do
      ...
      on host, 'cat /root/htaccess.txt', acceptable_exit_codes: 0 do
        ...
        expect(stdout).to match(%r{^user1:{SHA}CLub7iwpjkqz0enKLoRcbiDtUCo=$})
        expect(stdout).to match(%r{^user2:{SHA}yLo2mwINaPQsTgevY0gyfH9mxk4=$})
      end
    end
